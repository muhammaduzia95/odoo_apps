...
export class DevQuickAccessDialog extends Component {
    ...
    /**
     * Resolve a result to an Odoo 17 web URL. Odoo 17 uses the classic
     * hash-based web client routes, so we build "/web#..." URLs. We navigate
     * by URL (in a new tab) rather than doAction(), so the current view is
     * left untouched.
     */
    async _urlFor(res) {
        if (res.type === "app") {
            ...
            // We include menu_id so the app is highlighted, like clicking it.
            if (res.action) {
                const actionId = parseInt(res.action.split(",")[1], 10);
                return `/web#menu_id=${res.menuId}&action=${actionId}`;
            }
            ...
            if (action) {
                const actionId = parseInt(action.split(",")[1], 10);
                return `/web#menu_id=${res.menuId}&action=${actionId}`;
            }
            return "/web";
        }
        ...
        if (res.type === "action") {
            return `/web#action=${res.actionId}`;
        }
        // Model result: there's no action attached, so prefer a real window
        // action for this model; otherwise Odoo 17 resolves the raw-model hash
        // to a default list view natively.
